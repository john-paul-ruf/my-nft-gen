...
import { getRandomIntExclusive, getRandomIntInclusive, randomId } from 'my-nft-gen/src/core/math/random.js';
...
export class GatesEffect extends LayerEffect {
    ...
    static presets = [
        {
            ...
            currentEffectConfig: {
                ...
                gates: new Range(1, 2),
                numberOfSides: new Range(3, 4),
                innerColor: new ColorPicker(ColorPicker.SelectionType.colorBucket),
                outerColor: new ColorPicker(ColorPicker.SelectionType.colorBucket),
                thickness: 18,
                stroke: 0,
                accentRange: new DynamicRange(new Range(1, 3), new Range(6, 10)),
        ...
        },
        {
            ...
            currentEffectConfig: {
                ...
                center: new Position({x: 1080 / 2, y: 1920 / 2}),
                gates: new Range(3, 5),
                numberOfSides: new Range(5, 8),
                innerColor: new ColorPicker(ColorPicker.SelectionType.colorBucket),
                outerColor: new ColorPicker(ColorPicker.SelectionType.colorBucket),
                thickness: 30,
                stroke: 1,
                accentRange: new DynamicRange(new Range(4, 8), new Range(15, 25)),
                blurRange: new DynamicRange(new Range(2, 3), new Range(5, 7)),
    ...
    // quick fix
        for (let i = 0; i < context.data.numberOfGates; i++) {
            ...
            await context.canvas.drawPolygon2d(context.data.gates[i].radius, context.data.center.getPosition(context.currentFrame, context.numberOfFrames), context.data.numberOfSides, theAngleGaston, context.data.thickness, context.data.gates[i].color, context.data.stroke + theAccentGaston, context.data.gates[i].color);
        }
    ...
    async #draw(context, filename) {
        for (let i = 0; i < context.data.numberOfGates; i++) {
            ...
            const invert = direction <= 0;
            const theAngleGaston = (findOneWayValue(0, 360 / context.data.numberOfSides, 1, context.numberOfFrames, context.currentFrame, invert) + context.data.gates[i].startingAngle) % 360;
            await context.canvas.drawPolygon2d(context.data.gates[i].radius, context.data.center.getPosition(context.currentFrame, context.numberOfFrames), context.data.numberOfSides, theAngleGaston, context.data.thickness, context.data.gates[i].innerColor, 0, context.data.gates[i].innerColor);
        }
